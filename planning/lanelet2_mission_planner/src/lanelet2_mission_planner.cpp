...
void LaneletMissionPlanner::goal_pose_callback(const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
{
  ...
  if(ready_to_plan && lanelet_map_set)
  {
    ...
    geometry_msgs::msg::Pose refined_goal = LaneletMissionPlannerUtils::get_closest_centerline_pose(goal_pose);
    // Get start and goal lanelet
    lanelet::Lanelet start_lanelet, goal_lanelet;
    lanelet::utils::query::getClosestLaneletWithConstrains(road_lanelets, start_pose_, &start_lanelet, 0.0)
    lanelet::utils::query::getClosestLaneletWithConstrains(road_lanelets, refined_goal, &goal_lanelet, 0.0)

    // Use route graph to collect all lanelets
    Optional<routing::LaneletPath> shortestPath = routing_graph_ptr_->shortestPath(start_lanelet, goal_lanelet);
    ...
    path_msg.path_point = LaneletMissionPlannerUtils::generate_path_points(continuous_lane, refined_goal);
