...
{
  acados_ocp_capsule = nmpc_planner_steer_acados_create_capsule();
  // there is an opportunity to change the number of shooting intervals in C without new code generation
  N = NMPC_PLANNER_STEER_N;
  // allocate the array and fill it accordingly
  status = nmpc_planner_steer_acados_create_with_discretization(acados_ocp_capsule, N, new_time_steps);
  if (status)
  {
	...
	nlp_config = nmpc_planner_steer_acados_get_nlp_config(acados_ocp_capsule);
	nlp_dims = nmpc_planner_steer_acados_get_nlp_dims(acados_ocp_capsule);
  nlp_in = nmpc_planner_steer_acados_get_nlp_in(acados_ocp_capsule);
  nlp_out = nmpc_planner_steer_acados_get_nlp_out(acados_ocp_capsule);
  nlp_solver = nmpc_planner_steer_acados_get_nlp_solver(acados_ocp_capsule);
  nlp_opts = nmpc_planner_steer_acados_get_nlp_opts(acados_ocp_capsule);
}
...
{
	// Set goal such that we plan for 3s minimum
